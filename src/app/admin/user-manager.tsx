...
export function UserManager({ users, tenants }: { users: User[]; tenants: Tenant[] }) {
  ...
  async function remove(user: User) {
    if (!window.confirm(`Remove ${user.name || user.email} from ${user.tenantName}? Their agent access will also be revoked.`)) return;
  ...
  return <article id="client-users" className="glass mt-5 scroll-mt-6 rounded-2xl p-6">
    ...
    {(adding || editing) && <form onSubmit={submit} className="mt-5 grid gap-3 rounded-2xl border border-[#173f3310] bg-white/70 p-5 md:grid-cols-2 xl:grid-cols-4">
      <input type="hidden" name="userId" value={editing?.userId ?? ""}/><input type="hidden" name="tenantId" value={editing?.tenantId ?? ""} disabled={!editing}/>
      {!editing && <label className="text-xs font-semibold text-[#596a64]">Client workspace<select name="tenantId" required defaultValue="" className="mt-2 h-11 w-full rounded-xl border border-[#173f3320] bg-white px-3 text-sm"><option value="" disabled>Select client</option>{tenants.map((tenant) => <option key={tenant.id} value={tenant.id}>{tenant.name}</option>)}</select></label>}
