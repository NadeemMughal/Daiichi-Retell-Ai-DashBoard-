...
export function UserManager({ users, tenants }: { users: User[]; tenants: Tenant[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      formElement.reset(); setAdding(false); setEditing(null); setMessage(editing ? "User details updated." : "User created successfully."); router.refresh();
  ...
  return <article id="client-users" className="glass mt-5 scroll-mt-6 rounded-2xl p-4 sm:p-6">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="flex items-center gap-2 font-semibold"><UserRound className="size-5 text-[#1f7659]"/>Client users</h2><p className="mt-1 text-xs text-[#71817c]">Create and manage dashboard-only users. Agent visibility is assigned separately below.</p></div><button onClick={() => { setMessage(""); setEditing(null); setAdding(true); }} disabled={!tenants.length} className="flex h-10 items-center justify-center gap-2 rounded-xl bg-[#164f3e] px-4 text-sm font-semibold text-white disabled:opacity-40"><Plus className="size-4"/>Add new user</button></div>
    {message && <p role="status" aria-live="polite" className="mt-4 rounded-xl bg-[#e8f3ed] px-4 py-3 text-sm font-semibold text-[#1f7659]">{message}</p>}
    {(adding || editing) && <form key={editing?.userId ?? "new-user"} onSubmit={submit} className="mt-5 grid grid-cols-1 gap-3 rounded-2xl border border-[#173f3310] bg-white/70 p-5 md:grid-cols-2 xl:grid-cols-3">
      ...
      <label className="text-xs font-semibold text-[#596a64]">{editing ? "Reset password" : "Temporary password"}<input name="password" type="password" required={!editing} minLength={8} autoComplete="new-password" placeholder={editing ? "Leave blank to keep current" : "At least 8 characters"} className="mt-2 h-11 w-full rounded-xl border border-[#173f3320] bg-white px-3 text-sm"/></label>
      <div className="flex items-end gap-2"><button disabled={busy} className="flex h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-[#164f3e] px-4 text-sm font-semibold text-white">{busy && <LoaderCircle className="size-4 animate-spin"/>}{editing ? "Save changes" : "Create user"}</button><button type="button" onClick={() => { setMessage(""); setAdding(false); setEditing(null); }} className="grid size-11 place-items-center rounded-xl border border-[#173f3320] bg-white"><X className="size-4"/></button></div>
    </form>}
