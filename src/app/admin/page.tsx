  ...
  return <main className="subtle-grid min-h-screen p-5 md:p-10">
    <div className="mx-auto max-w-7xl">
      ...
      <section id="agent-ownership" className="mt-5 scroll-mt-6 grid gap-5 xl:grid-cols-[1.4fr_1fr]">
        <article className="glass rounded-2xl p-6"><div className="flex items-center justify-between"><div><h2 className="font-semibold">Agent ownership</h2><p className="mt-1 text-xs text-[#71817c]">Each Retell agent has one client boundary and any number of user-level grants inside that client.</p></div><Activity className="size-5 text-[#1f7659]" /></div><div className="mt-5 space-y-3">{(agents ?? []).slice(0, 10).map((agent) => { const assignment = agent.agent_assignments?.find((item) => !item.valid_to); const tenant = assignment && (Array.isArray(assignment.tenants) ? assignment.tenants[0] : assignment.tenants); const grantedUsers = grantsByAgent.get(agent.id) ?? []; return <div key={agent.id} className="rounded-xl border border-[#173f3310] bg-white/70 p-4"><div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-lg bg-[#e8f3ed] text-[#1f7659]"><Bot className="size-4" /></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{agent.display_name}</p><p className="text-xs capitalize text-[#84928d]">{agent.kind} · {grantedUsers.length} user {grantedUsers.length === 1 ? "grant" : "grants"}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${tenant ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{tenant?.display_name ?? "Client assignment required"}</span></div><div className="mt-3 flex flex-wrap gap-2">{grantedUsers.map((user) => <span key={`${agent.id}:${user.userId}`} className="rounded-full border border-emerald-100 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-800" title={`${user.email} · ${user.tenantName}`}>{user.name}</span>)}{tenant && !grantedUsers.length && <span className="text-xs text-[#84928d]">Assigned to the client, but no end user currently has access.</span>}{!tenant && <span className="text-xs font-semibold text-amber-700">Granting access will create the client assignment after server validation.</span>}</div></div>})}{!agents?.length && <p className="rounded-xl bg-white/60 p-8 text-center text-sm text-[#71817c]">No agents imported yet.</p>}</div></article>
      ...
      {context.platformRoles.includes("super_admin") && <PlatformAdminManager admins={(platformAdmins ?? []).map((row) => { const user = Array.isArray(row.user) ? row.user[0] : row.user; return { userId: row.user_id, name: user?.display_name ?? user?.email ?? "Administrator", email: user?.email ?? "" }; })} />}
