...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ ownership?: string }> }) {
  ...
  return <main className="subtle-grid min-h-screen p-5 md:p-10">
    <div className="mx-auto max-w-7xl">
      ...
      {context.permissions.has("platform.manage") && <PlatformAdminManager canManageSuperAdmins={context.permissions.has("super_admin.manage")} admins={(platformAdmins ?? []).map((row) => { const user = Array.isArray(row.user) ? row.user[0] : row.user; return { userId: row.user_id, name: user?.display_name ?? user?.email ?? "Administrator", email: user?.email ?? "", role: row.role as "super_admin" | "operations_admin" }; })} />}
