...
export function AgentOwnershipList({ agents, workspaces }: { agents: Agent[]; workspaces: Workspace[] }) {
  ...
  const visibleAgents = useMemo(() => agents.filter((agent) => workspaceId === "all" || agent.tenantId === workspaceId), [agents, workspaceId]);
  ...
  return <article className="glass rounded-2xl p-6">
    ...
    <label className="relative mt-5 block"><span className="mb-2 block text-xs font-semibold text-[#596a64]">Filter by workspace</span><select value={workspaceId} onChange={(event) => setWorkspaceId(event.target.value)} className="h-11 w-full appearance-none rounded-xl border border-[#173f3320] bg-white px-4 pr-10 text-sm font-semibold text-[#164f3e] outline-none transition focus:border-[#1f7659] focus:ring-4 focus:ring-[#1f765915]"><option value="all">All workspaces</option>{workspaces.map((workspace) => <option key={workspace.id} value={workspace.id}>{workspace.name}</option>)}</select><ChevronDown className="pointer-events-none absolute bottom-3.5 right-4 size-4 text-[#71817c]"/></label>
