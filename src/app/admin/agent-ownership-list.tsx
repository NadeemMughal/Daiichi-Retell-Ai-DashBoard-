...
export function AgentOwnershipList({ agents, workspaces, initialAssignmentFilter = "all" }: { agents: Agent[]; workspaces: Workspace[]; initialAssignmentFilter?: AssignmentFilter }) {
  ...
  const visibleAgents = useMemo(() => agents.filter((agent) => (workspaceId === "all" || agent.tenantId === workspaceId) && (assignmentFilter === "all" || (assignmentFilter === "assigned" ? Boolean(agent.tenantId) : !agent.tenantId))), [agents, assignmentFilter, workspaceId]);
