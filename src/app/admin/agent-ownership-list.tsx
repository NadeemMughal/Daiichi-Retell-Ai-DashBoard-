...
export function AgentOwnershipList({ agents, workspaces }: { agents: Agent[]; workspaces: Workspace[] }) {
  const [workspaceId, setWorkspaceId] = useState("all");
  const visibleAgents = useMemo(() => agents.filter((agent) => workspaceId === "all" || (workspaceId === "unassigned" ? !agent.tenantId : agent.tenantId === workspaceId)), [agents, workspaceId]);
  const selectedName = workspaceId === "all" ? "All workspaces" : workspaceId === "unassigned" ? "Unassigned agents" : workspaces.find((workspace) => workspace.id === workspaceId)?.name ?? "Workspace";
  ...
  return <article className="glass rounded-2xl p-6">
    ...
    <label className="relative mt-5 block"><span className="mb-2 block text-xs font-semibold text-[#596a64]">Filter by workspace</span><select value={workspaceId} onChange={(event) => setWorkspaceId(event.target.value)} className="h-11 w-full appearance-none rounded-xl border border-[#173f3320] bg-white px-4 pr-10 text-sm font-semibold text-[#164f3e] outline-none transition focus:border-[#1f7659] focus:ring-4 focus:ring-[#1f765915]"><option value="all">All workspaces ({agents.length})</option>{workspaces.map((workspace) => <option key={workspace.id} value={workspace.id}>{workspace.name} ({agents.filter((agent) => agent.tenantId === workspace.id).length})</option>)}<option value="unassigned">Unassigned agents ({agents.filter((agent) => !agent.tenantId).length})</option></select><ChevronDown className="pointer-events-none absolute bottom-3.5 right-4 size-4 text-[#71817c]"/></label>
    <div className="mt-3 flex items-center justify-between text-xs text-[#71817c]"><span>{selectedName}</span><span>{visibleAgents.length} {visibleAgents.length === 1 ? "agent" : "agents"}</span></div>
    <div className="mt-3 max-h-[430px] space-y-3 overflow-y-auto pr-2 [scrollbar-color:#b8c9c2_transparent]">
      ...
      {!visibleAgents.length && <p className="rounded-xl border border-dashed border-[#173f3320] bg-white/50 p-8 text-center text-sm text-[#71817c]">No agents are assigned to this workspace.</p>}
    </div>
  </article>;
}
