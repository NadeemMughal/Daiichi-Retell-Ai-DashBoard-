...
export function AgentOwnershipList({ agents, workspaces, initialAssignmentFilter = "all" }: { agents: Agent[]; workspaces: Workspace[]; initialAssignmentFilter?: AssignmentFilter }) {
  ...
  return <article className="glass rounded-2xl p-6">
    ...
    <div className="mt-3 max-h-[430px] space-y-3 overflow-y-auto pr-2 [scrollbar-color:#b8c9c2_transparent]">
      {visibleAgents.map((agent) => <div key={agent.id} className="rounded-xl border border-[#173f3310] bg-white/70 p-4"><div className="flex flex-wrap items-center gap-x-3 gap-y-2"><div className="grid size-9 shrink-0 place-items-center rounded-lg bg-[#e8f3ed] text-[#1f7659]"><Bot className="size-4"/></div><div className="min-w-0 flex-1 basis-40"><p className="truncate text-sm font-semibold">{agent.name}</p><p className="text-xs capitalize text-[#84928d]">{agent.kind} · {agent.grants.length} user {agent.grants.length === 1 ? "grant" : "grants"}</p></div><span className={`max-w-full shrink-0 truncate rounded-full px-2.5 py-1 text-xs font-semibold sm:max-w-52 ${agent.tenantName ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`} title={agent.tenantName ?? "Not assigned to a client workspace yet. The next Retell sync assigns it."}>{agent.tenantName ?? "Assignment pending sync"}</span></div><div className="mt-3 flex flex-wrap gap-2">{agent.grants.map((user) => <span key={`${agent.id}:${user.userId}`} className="rounded-full border border-emerald-100 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-800" title={user.email}>{user.name}</span>)}{!agent.grants.length && <span className="text-xs text-[#84928d]">No client user currently has access.</span>}</div></div>)}
