...
function TimeBoundary({ label, value, onChange, utcOffset, onUtcOffsetChange }: { label: string; value: string; onChange: (value: string) => void; utcOffset: string; onUtcOffsetChange: (value: string) => void }) { return <fieldset className="mt-4"><legend className="text-[11px] font-semibold text-[#596a7a]">{label}</legend><div className="mt-1 flex h-11 overflow-hidden rounded-xl border border-[#d9dee7] bg-white focus-within:border-[#4f647e] focus-within:ring-4 focus-within:ring-[#4f647e12]"><input aria-label={label} type="time" step="1" value={value} onChange={(event) => onChange(event.target.value)} className="min-w-0 flex-1 bg-transparent px-3 text-sm text-[#253246] outline-none"/><select aria-label={`${label} UTC offset`} value={utcOffset} onChange={(event) => onUtcOffsetChange(event.target.value)} className="max-w-28 border-l border-[#d9dee7] bg-[#f5f7f9] px-2 text-xs font-semibold text-[#667487] outline-none"><option value="+00:00">UTC</option>{UTC_OFFSETS.filter((offset) => offset !== "+00:00").map((offset) => <option key={offset} value={offset}>UTC{offset}</option>)}</select></div></fieldset>; }
