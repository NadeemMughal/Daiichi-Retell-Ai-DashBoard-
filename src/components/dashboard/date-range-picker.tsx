...
export function DateRangePicker({ value, onChange, minimum, buttonLabel, align = "right" }: { value: DateRangeValue; onChange: (value: DateRangeValue) => void; minimum?: string; buttonLabel?: string; align?: "left" | "right" }) {
  ...
  function choosePreset(start: string) { setDraftStart(normalized(start)); setDraftEnd(maximum); setSelectingEnd(false); setVisibleMonth(addMonths(parseDate(maximum), -1)); }
  ...
  function apply() { if (!draftStart || !draftEnd || draftStart > draftEnd) return; const preset = presets.find((item) => normalized(item.start) === draftStart && maximum === draftEnd); onChange({ start: draftStart, end: draftEnd, label: preset?.label ?? `${formatDate(draftStart)} – ${formatDate(draftEnd)}` }); setOpen(false); }
...
function TimeBoundary({ value }: { value: string }) { return <div className="mt-4 flex h-10 items-center rounded-xl border border-[#d9dee7] bg-white px-3 text-sm"><span className="mr-2 text-[#84928d]">◷</span><span>{value}</span><span className="ml-auto rounded bg-[#f1f3f5] px-2 py-1 text-xs text-[#667487]">UTC+05</span></div>; }
